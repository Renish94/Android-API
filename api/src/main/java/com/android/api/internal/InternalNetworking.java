...
import com.android.api.interceptors.HttpLoggingInterceptor;
...
import static com.android.api.common.Method.DELETE;
import static com.android.api.common.Method.GET;
import static com.android.api.common.Method.HEAD;
import static com.android.api.common.Method.OPTIONS;
import static com.android.api.common.Method.PATCH;
import static com.android.api.common.Method.POST;
import static com.android.api.common.Method.PUT;
...
public final class InternalNetworking {
    ...
    public static Response performSimpleRequest(APIRequest request) throws APIError {
        ...
        try {
            ...
            switch (request.getMethod()) {
                ...
                case OPTIONS: {
                    builder = builder.method(APIConstants.OPTIONS, null);
                    break;
                }
                case PATCH: {
                    requestBody = request.getRequestBody();
            ...
            throw new APIError(ioe);
        }
        return okHttpResponse;
    }

    public static Response performDownloadRequest(final APIRequest request) throws APIError {
        Request okHttpRequest;
        Response okHttpResponse;
        try {
            Request.Builder builder = new Request.Builder().url(request.getUrl());
    ...
    public static Response performUploadRequest(APIRequest request) throws APIError {
        Request okHttpRequest;
        Response okHttpResponse;
        try {
            Request.Builder builder = new Request.Builder().url(request.getUrl());
    ...
    public static void addHeadersToRequestBuilder(Request.Builder builder, APIRequest request) {
        if (request.getUserAgent() != null) {
            builder.addHeader(APIConstants.USER_AGENT, request.getUserAgent());
        } else if (sUserAgent != null) {
            request.setUserAgent(sUserAgent);
            builder.addHeader(APIConstants.USER_AGENT, sUserAgent);
        }
        Headers requestHeaders = request.getHeaders();
        if (requestHeaders != null) {
            builder.headers(requestHeaders);
            if (request.getUserAgent() != null && !requestHeaders.names().contains(APIConstants.USER_AGENT)) {
                builder.addHeader(APIConstants.USER_AGENT, request.getUserAgent());
            }
        }
    }
    ...
    public static void setClientWithCache(Context context) {
        sHttpClient = new OkHttpClient().newBuilder()
                .cache(Utils.getCache(context, APIConstants.MAX_CACHE_SIZE, APIConstants.CACHE_DIR_NAME))
                .connectTimeout(60, TimeUnit.SECONDS)
                .readTimeout(60, TimeUnit.SECONDS)
                .writeTimeout(60, TimeUnit.SECONDS)
                .build();
