...
import com.android.api.interfaces.DownloadListener;
import com.android.api.interfaces.DownloadProgressListener;
import com.android.api.interfaces.JSONArrayRequestListener;
import com.android.api.interfaces.JSONObjectRequestListener;
import com.android.api.interfaces.OkHttpResponseAndBitmapRequestListener;
import com.android.api.interfaces.OkHttpResponseAndJSONArrayRequestListener;
import com.android.api.interfaces.OkHttpResponseAndJSONObjectRequestListener;
import com.android.api.interfaces.OkHttpResponseAndParsedRequestListener;
import com.android.api.interfaces.OkHttpResponseAndStringRequestListener;
import com.android.api.interfaces.OkHttpResponseListener;
...
import com.android.api.internal.SynchronousCall;
...
public class APIRequest<T extends APIRequest> {
    ...
    public APIRequest(GetRequestBuilder builder) {
    ...
    public APIRequest(PostRequestBuilder builder) {
        this.mRequestType = RequestType.SIMPLE;
        this.mMethod = builder.mMethod;
        this.mPriority = builder.mPriority;
        this.mUrl = builder.mUrl;
    ...
    public APIRequest(DownloadBuilder builder) {
    ...
    public APIRequest(MultiPartBuilder builder) {
    ...
    public APIResponse executeForJSONObject() {
        this.mResponseType = ResponseType.JSON_OBJECT;
        return SynchronousCall.execute(this);
    }

    public APIResponse executeForJSONArray() {
        this.mResponseType = ResponseType.JSON_ARRAY;
        return SynchronousCall.execute(this);
    }

    public APIResponse executeForString() {
        this.mResponseType = ResponseType.STRING;
        return SynchronousCall.execute(this);
    }

    public APIResponse executeForOkHttpResponse() {
        this.mResponseType = ResponseType.OK_HTTP_RESPONSE;
        return SynchronousCall.execute(this);
    }

    public APIResponse executeForBitmap() {
        this.mResponseType = ResponseType.BITMAP;
        return SynchronousCall.execute(this);
    }

    public APIResponse executeForParsed(TypeToken typeToken) {
        this.mType = typeToken.getType();
        this.mResponseType = ResponseType.PARSED;
        return SynchronousCall.execute(this);
    }

    public APIResponse executeForObject(Class objectClass) {
        this.mType = objectClass;
        this.mResponseType = ResponseType.PARSED;
        return SynchronousCall.execute(this);
    }

    public APIResponse executeForObjectList(Class objectClass) {
        this.mType = $Gson$Types.newParameterizedTypeWithOwner(null, List.class, objectClass);
        this.mResponseType = ResponseType.PARSED;
        return SynchronousCall.execute(this);
    }

    public APIResponse executeForDownload() {
        return SynchronousCall.execute(this);
    }
    ...
    public APIResponse parseResponse(Response response) {
        switch (mResponseType) {
            case JSON_ARRAY:
                try {
                    ...
                    return APIResponse.failed(Utils.getErrorForParse(new APIError(e)));
                }
            case JSON_OBJECT:
                try {
            ...
            case STRING:
                try {
                    return APIResponse.success(Okio.buffer(response
            ...
            case PARSED:
                try {
                    return APIResponse.success(ParseUtil.getParserFactory()
            ...
            case PREFETCH:
                try {
                    ...
                    return APIResponse.success(APIConstants.PREFETCH);
                } catch (Exception e) {
                    return APIResponse.failed(Utils.getErrorForParse(new APIError(e)));
                }
        }
    ...
    public APIError parseNetworkError(APIError apiError) {
        try {
            if (apiError.getResponse() != null && apiError.getResponse().body() != null
                    && apiError.getResponse().body().source() != null) {
                apiError.setErrorBody(Okio.buffer(apiError
    ...
    public synchronized void deliverError(APIError apiError) {
        try {
            if (!isDelivered) {
                if (isCancelled) {
                    apiError.setCancellationMessageInError();
    ...
    private void deliverErrorResponse(APIError apiError) {
        if (mJSONObjectRequestListener != null) {
            mJSONObjectRequestListener.onError(apiError);
        } else if (mJSONArrayRequestListener != null) {
            mJSONArrayRequestListener.onError(apiError);
        } else if (mStringRequestListener != null) {
            mStringRequestListener.onError(apiError);
        } else if (mBitmapRequestListener != null) {
            mBitmapRequestListener.onError(apiError);
        } else if (mParsedRequestListener != null) {
            mParsedRequestListener.onError(apiError);
        } else if (mOkHttpResponseListener != null) {
            mOkHttpResponseListener.onError(apiError);
        } else if (mOkHttpResponseAndJSONObjectRequestListener != null) {
            mOkHttpResponseAndJSONObjectRequestListener.onError(apiError);
        } else if (mOkHttpResponseAndJSONArrayRequestListener != null) {
            mOkHttpResponseAndJSONArrayRequestListener.onError(apiError);
        } else if (mOkHttpResponseAndStringRequestListener != null) {
            mOkHttpResponseAndStringRequestListener.onError(apiError);
        } else if (mOkHttpResponseAndBitmapRequestListener != null) {
            mOkHttpResponseAndBitmapRequestListener.onError(apiError);
        } else if (mOkHttpResponseAndParsedRequestListener != null) {
            mOkHttpResponseAndParsedRequestListener.onError(apiError);
        } else if (mDownloadListener != null) {
            mDownloadListener.onError(apiError);
        }
    }
